...
import etc.method.item.MethodItemReturnTest;
import etc.method.item.MethodItemTest;
...
//        new MethodTest().start(null, new String[]{"메소드명[] (test,1,2)"}, repository);
    ...
    public void start(String line, String[] params,
        ...
        if (returnValue == null) repositoryArray.get(0).get(METHOD).put(methodName, new MethodItemTest(methodParameters, finalTotal, fileName));
        else repositoryArray.get(0).get(METHOD).put(methodName, new MethodItemReturnTest(methodParameters, finalTotal, fileName, returnValue));
    }
