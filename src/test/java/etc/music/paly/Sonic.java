...
import static etc.music.paly.MusicRepository.musicRepository;
...
public class Sonic {
    ...
    public void start(File file, float speed, float volume, float pitch, float rate)
            throws UnsupportedAudioFileException, IOException, LineUnavailableException {
        try (AudioInputStream stream = AudioSystem.getAudioInputStream(file)) {
            start(stream, speed, volume, pitch, rate);
        }
    }

    public void start(AudioInputStream stream, float speed, float volume, float pitch, float rate)
            throws LineUnavailableException, IOException {
        int quality = 0;
        boolean emulateChordPitch = false;
        start(stream, speed, pitch, rate, volume, emulateChordPitch, quality);
    }

    public void start(AudioInputStream stream, float speed, float pitch, float rate, float volume,
                      boolean emulateChordPitch, int quality) throws LineUnavailableException, IOException {
        ...
        try (SourceDataLine line = (SourceDataLine) AudioSystem.getLine(info)) {
            ...
            line.open(musicRepository.get("레").getStream().getFormat());

            line.start();
            start(stream, line, speed, pitch, rate, volume, emulateChordPitch, quality, sampleRate, numChannels);
            line.drain();
            line.stop();
    ...
    private void start(AudioInputStream audioStream, SourceDataLine line,
                      float speed, float pitch, float rate, float volume,
                      boolean emulateChordPitch,
                      int quality, int sampleRate, int numChannels) throws IOException {
        ...
        setRate(rate);
        setVolume(volume);
        setChordPitch(emulateChordPitch);
        setQuality(quality);
        do {
    ...
    private short[] inputBuffer, outputBuffer, pitchBuffer, downSampleBuffer;
    private float speed, volume, pitch, rate;
    private int oldRatePosition, newRatePosition;
    private boolean useChordPitch;
    private int quality;
    private int numChannels;
    private int inputBufferSize, pitchBufferSize;
    private int outputBufferSize;
    private int numInputSamples, numOutputSamples, numPitchSamples;
    private int minPeriod, maxPeriod;
    private int maxRequired;
    private int remainingInputToCopy;
    private int sampleRate;
    private int prevPeriod, prevMinDiff;
    private int minDiff, maxDiff;
    ...
    private void overlapAdd(int numSamples, int numChannels, short[] out,
                            int outPos, short[] rampDown,
                            int rampDownPos, short[] rampUp, int rampUpPos) {
    ...
    private void overlapAddWithSeparation(
            int numSamples, int numChannels, int separation,
            short[] out, int outPos, short[] rampDown,
            int rampDownPos, short[] rampUp, int rampUpPos) {
        for(int i = 0; i < numChannels; i++) {
            int o = outPos*numChannels + i;
            int u = rampUpPos*numChannels + i;
    ...
    private short interpolate(short[] in, int inPos, int oldSampleRate, int newSampleRate) {
        ...
        if (overflowCount > 0) return Short.MAX_VALUE;
        else if (overflowCount < 0) return Short.MIN_VALUE;
    ...
    private void processStreamInput() {
        ...
        if(!useChordPitch) r *= pitch;
        if(s > 1.00001 || s < 0.99999) changeSpeed(s);
        else {
            copyToOutput(inputBuffer, 0, numInputSamples);
            numInputSamples = 0;
        }
        if(useChordPitch) {
            if(pitch != 1.0f) adjustPitch(originalNumOutputSamples);
