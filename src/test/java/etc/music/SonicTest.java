...
public class SonicTest {
    private static SonicTest sonicTest;
    public static SonicTest getInstance() {
        if (sonicTest == null) {
            synchronized (SonicTest.class) {
                sonicTest = new SonicTest();
            }
        }
        return sonicTest;
    }

    public void start(MusicItemTest item)
            throws LineUnavailableException, UnsupportedAudioFileException, IOException {
        for (int i = 0; i < item.getLoop(); i++) {
            AudioInputStream stream = item.getStream();
            AudioFormat format = stream.getFormat();
            int sampleRate = (int) format.getSampleRate();
            int numChannels = format.getChannels();
            SourceDataLine.Info info = new DataLine.Info(
                    SourceDataLine.class, format, ((int)stream.getFrameLength() * format.getFrameSize()));
            SourceDataLine line = (SourceDataLine)AudioSystem.getLine(info);
            line.open(stream.getFormat());
            line.start();
            play(stream, line, item.getSpeed(),
                    item.getPitch(), item.getRate(),
                    item.getVolume(), item.isEmulateChordPitch(),
                    item.getQuality(), sampleRate, numChannels);
            line.drain();
            line.stop();
        }
    }

    private void play(AudioInputStream audioStream, SourceDataLine line,
                      float speed, float pitch, float rate, float volume,
                      boolean emulateChordPitch, int quality,
                      int sampleRate, int numChannels) throws IOException {
        ...
        setRate(rate);
        setVolume(volume);
        setUseChordPitch(emulateChordPitch);
        setQuality(quality);
        do {
