...
public class PoisonMethod implements PoisonTools, StartWork {
    private final String text;
    private final Pattern pattern;
    private final HttpMethod httpMethod;

    public PoisonMethod(String className, String type, HttpMethod httpMethod) {
        this.httpMethod = httpMethod;
        String or = blackMerge(BL, orMerge(TOTAL_LIST) + BLANKS + VARIABLE_NAME + VARIABLE_PUT + "\\S+", BR);
        this.text = className + ACCESS + type;
    ...
    public void start(String line, String origen,
        ...
        String[] tokens = matchSplitError(
                matchSplitError(line.strip(), text, 2)[1],
                "(?!" + BR + ")" + BLANKS + "(?=" + SL + ")", 2);
        ...
        Poison.httpServerManager.addMethod(httpMethod, token[0], getTotal(total[0]), getParams(token), total[1]);
    }
